...
const handleFiles = async (files: File[]) => {
  ...
  for (const file of files) {
    ...
    if (!supportedTypes.includes(file.type)) {
      window.$toast.error(`File ${file.name} is not supported. Please upload images (JPEG, PNG, GIF, WebP, SVG) or videos (MP4, MPEG, MOV, AVI, WebM).`);
      continue;
    }
    ...
    if (file.size > maxSize) {
      window.$toast.error(`File ${file.name} is too large. Maximum size is 20MB.`);
      continue;
    }
